...
export const ToastProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const info = useCallback((msg: string, title?: string) => showToast(msg, 'info', title || 'Notice'), [showToast]);
        ...
        <AnimatePresence>
          {toasts.map((t) => (
            ...
            >
              ...
              {t.type === 'info' && <Info className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />}

              <div className="flex-1 text-sm">
                {t.title && <p className="font-semibold text-xs tracking-wider uppercase mb-0.5">{t.title}</p>}
                <p className="font-medium">{t.message}</p>
              </div>

              <button
                onClick={() => removeToast(t.id)}
                className="text-stone-400 hover:text-stone-700 transition p-0.5 rounded-lg"
              >
                <X className="w-4 h-4" />
              </button>
            </motion.div>
